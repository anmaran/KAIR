...
# --------------------------------------------
# get uint16 image of size HxWxn_channles (RGB)
# --------------------------------------------
...
def imread_uint16(path, n_channels=3):
    ...
    # output: HxWx3(RGB or GGG), or HxWx1 (G)
    if n_channels == 1:
        img = tifffile.imread(path)  # cv2.IMREAD_GRAYSCALE
        img = np.expand_dims(img, axis=2)  # HxWx1
    elif n_channels == 3:
        img = tifffile.imread(path)  # BGR or G
    return img
...
# --------------------------------------------
'''
def ergas_mine(org_np, new_np, r=2):
    t = []
    for i in range (0, org_np.shape[2]):
        rmse = fr.rmse(org_np[:,:,i], new_np[:,:,i])
        mean_org = np.mean(org_np[:,:,i])
        d = rmse**2 / mean_org**2
        t.append(d)
        suma = (sum(t))
    ergas = 100 * 1/r * (1/org_np.shape[2] * suma)**(1/2)
    return ergas
